...
export function checkSiteLevel(pages: CrawlResult[]): SEOCheck[] {
  ...
  if (duplicateTitles.length > 0) {
    checks.push({
      ...
      severity: 'warning',
      impact: -10,
      message: `Duplicate titles found across ${duplicateTitles.length} groups of pages`,
  ...
  if (duplicateMetas.length > 0) {
    checks.push({
      ...
      severity: 'warning',
      impact: -10,
      message: `Duplicate meta descriptions found across ${duplicateMetas.length} groups of pages`,
